...
CREATE TABLE IF NOT EXISTS `pre_go_acc_user_verify_9999` (
  ...
  `verify_key` VARCHAR(255) NOT NULL,         -- User's email (or phone number) to identify the OTP recipient
  `verify_key_hash` VARCHAR(255) NOT NULL,    -- Hash of verify_key for security
  ...
  INDEX `idx_verify_otp` (`verify_otp`),      -- Create an index for the verify_otp field
